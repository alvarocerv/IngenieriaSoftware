...
def iniciar_separacion(
        ...
        try:
            ...
            if val:
                ...
                if 0 < train_pct < 100:
                    ...
                    label_test_pct.config(
                        text=f"Porcentaje de Test: {test_pct:.1f}%"
                    )
                else:
    ...
    # Campos de entrada
    ttk.Label(frame_inputs, text="Porcentaje de Entrenamiento:").grid(
        row=0, column=0, padx=5, pady=5, sticky="w"
    )
    entry_train_pct = ttk.Entry(
        frame_inputs, textvariable=train_pct_var, width=10
    )
    ...
    ttk.Label(
        frame_inputs, text="Semilla (para la separación aleatoria):"
    ...
    df_original_completo = (
        df_original if df_original is not None else df_procesado
    )
        ...
        try:
            ...
            if not train_pct_str:
                messagebox.showerror(
                    "Error", "Debes rellenar al menos el campo de porcentaje."
                )
                return
            ...
            if seed_str.strip():
                try:
                    ...
                    messagebox.showerror(
                        "Error",
                        "La semilla aleatoria debe ser un número entero.",
                    )
                    return
            ...
            if len(df_procesado) < 5:
                messagebox.showerror(
                    "Error",
                    "No hay suficientes datos para realizar la separación (mínimo 5 filas).",
                )
                return

            train_df, test_df = train_test_split(
                df_procesado, test_size=test_pct / 100, random_state=seed
            )
            ...
            btn_train = ttk.Button(
                frame_vista, text="Ver Conjunto de Entrenamiento"
            )
    ...
    btn_separar = ttk.Button(
        frame_pasos_container, text="Separar Datos", command=separar_datos
    )
