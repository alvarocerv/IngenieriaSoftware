...
def guardar_modelo(modelo, input_cols, output_col, descripcion, metricas):
    ...
    try:
        ...
        if not file_path:
            messagebox.showinfo(
                "Guardado cancelado", "El guardado fue cancelado."
            )
            return
        ...
        formula = (
            ...
            + " + ".join(
                [
                    f"({coef:.6f} * {col})"
                    for coef, col in zip(modelo.coef_, input_cols)
                ]
            )
        ...
        messagebox.showinfo(
            "Modelo guardado",
            f"Guardado correctamente {desc_flag} en:\n{file_path}",
        )
...
def cargar_modelo(notebook_visor, frame_pasos_container):
    ...
    if ruta.endswith(".json"):
        try:
            ...
            messagebox.showerror(
                "Error", f"El archivo no es válido o está corrupto:\n{e}"
            )
            return
        ...
        messagebox.showerror(
            ...
            "Solo se soportan archivos .json, .joblib y .pkl",
        )
        return
    ...
    ttk.Label(
        frame, text="Modelo Recuperado", font=("Arial", 14, "bold")
    ).pack(pady=10)

    # Descripción
    ttk.Label(frame, text="Descripción:", font=("Arial", 11, "bold")).pack(
        anchor="w"
    )
    ttk.Label(frame, text=info["descripcion"], wraplength=800).pack(
        anchor="w", pady=(0, 10)
    )

    # Fórmula
    ttk.Label(frame, text="Fórmula:", font=("Arial", 11, "bold")).pack(
        anchor="w"
    )
    ttk.Label(frame, text=info["formula"], wraplength=800).pack(
        anchor="w", pady=(0, 10)
    )

    # Coeficientes
    ttk.Label(frame, text="Coeficientes:", font=("Arial", 11, "bold")).pack(
        anchor="w"
    )
    for col, c in zip(info["entradas"], info["coeficientes"]):
        ttk.Label(frame, text=f"{col}: {c:.6f}").pack(anchor="w")

    ttk.Label(frame, text=f"Intercepto: {info['intercepto']:.6f}").pack(
        anchor="w", pady=(0, 10)
    )

    # Métricas
    ttk.Label(frame, text="Métricas:", font=("Arial", 11, "bold")).pack(
        anchor="w"
    )
    ...
    ttk.Label(
        frame, text="Predicción Interactiva", font=("Arial", 12, "bold")
    ).pack(anchor="w", pady=(15, 5))

    frame_pred = ttk.LabelFrame(
        frame, text="Ingrese valores para predecir", padding=10
    )
    ...
    def ejecutar_prediccion():
        ...
        try:
            ...
            valores = [
                float(input_entries[col].get()) for col in info["entradas"]
            ]
            pred = (
                sum(v * c for v, c in zip(valores, info["coeficientes"]))
                + info["intercepto"]
            )
    ...
    # Confirmación
    messagebox.showinfo(
        "Modelo cargado", "El modelo fue recuperado exitosamente."
    )
