...
def dibujar_grafico_predicciones(tab_grafico, modelo, df_test, nombre_col_entrada, nombre_col_salida):
    ...
    ax.scatter(x_test, y_pred, color='red', label='Predición', alpha=0.7)
    ...
    ax.plot(x_line, y_line, color='green', label='Liña de axuste', linewidth=2)

    # (Opcional) Mostrar fórmula en el título extendido
    ax.set_title(f'Predicións vs Test\nAxuste: y = {slope:.4f}x + {intercept:.4f}')

    ax.set_xlabel(nombre_col_entrada)
    ax.set_ylabel(nombre_col_salida)
    ax.set_title('Predicións vs Test')
    ax.legend()
    ax.grid(True)
