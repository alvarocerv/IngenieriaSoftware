...
class TestPreprocesadoDatosInexistentes:
    ...
    def test_rellenar_con_media(self, datos_con_nan):
        ...
        resultado[cols_numericas] = resultado[cols_numericas].fillna(resultado[cols_numericas].mean())
        ...
        assert resultado.loc[2, 'col1'] == pytest.approx(media_col1_original, rel=1e-5)
    
    def test_rellenar_con_mediana(self, datos_con_nan):
        """Prueba rellenar valores faltantes con la mediana"""
        resultado = datos_con_nan.copy()
        cols_numericas = resultado.select_dtypes(include=np.number).columns
        resultado[cols_numericas] = resultado[cols_numericas].fillna(resultado[cols_numericas].median())
        ...
        assert resultado.loc[2, 'col1'] == pytest.approx(mediana_col1_original, rel=1e-5)
